...
from ..callbacks.clr import LRFinder, OneCycleLR
...
def create_callbacks(
    ...
    if args.snapshots:
        ...
        checkpoint = keras.callbacks.ModelCheckpoint(
            os.path.join(
                args.snapshot_path,
                "{backbone}_{dataset_type}.h5".format(
...
def create_generators(args, preprocess_image):
    ...
    if args.dataset_type == "coco":
        ...
        validation_generator = CocoGenerator(args.coco_path, "val2017", **common_args)
        ...
        validation_generator = PascalVocGenerator(
            args.pascal_path, "test", **common_args
        )
        ...
        if args.val_annotations:
            validation_generator = CSVGenerator(
                args.val_annotations, args.classes, **common_args
            )
        else:
        ...
        validation_generator = KittiGenerator(
            args.kitti_path, subset="val", **common_args
        )
    else:
...
def parse_args(args):
    ...
    parser.add_argument(
        "--max-queue-size",
        help="Queue length for multiprocessing workers in fit generator.",
        type=int,
        default=10,
    )
